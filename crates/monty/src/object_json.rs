...
/// Body of an `$extension_handle` tag — mirrors the `MontyObject::ExtensionHandle`
/// fields so the JSON round-trip preserves enough information to re-resolve the
/// handle against its owning extension registry.
#[derive(Serialize)]
struct ExtensionHandleBody<'a> {
    registry_index: u16,
    type_name: &'a str,
    handle_id: u64,
}
