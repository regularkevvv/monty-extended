// Nightly clippy's `absolute_paths` lint fires on the `::pyo3::types::PyType`
// path generated by pyo3's `#[classmethod]` macro in `acreate` below. The lint
// span points at user source but `#[expect]` on the impl/fn/parameter doesn't
// catch it, and stable clippy doesn't fire the lint at all (so `#[expect]`
// would be unfulfilled). A scoped `#[allow]` is the only working escape hatch.
...
use ::monty::{
    ExcType, ExtFunctionResult, ExtensionRegistry, FunctionCall, JsonMontyArray, JsonMontyObject, JsonMontyPairs,
    LimitedTracker, MontyException, MontyObject, MontyRun, NameLookupResult, NoLimitTracker, OsCall, ReplFunctionCall,
    ReplNameLookup, ReplOsCall, ReplProgress, ReplResolveFutures, ReplStartError, ResolveFutures, ResourceTracker,
    RunProgress,
};
...
impl PyMonty {
    ...
    fn new(
        ...
        // When extensions are provided, take the synchronous, extension-aware
        // build path: collect extension type stubs, build the runner with
        // registered extensions, and attach host callables.
        if let Some(ext_list) = extensions
            && !ext_list.is_empty()
        {
            let (code_str, input_names, dc_registry) = inputs.into_extension_parts();

            if type_check {
                let ext_stubs = collect_extension_type_stubs(ext_list)?;
                let combined_stubs = combine_type_stubs(type_check_stubs, ext_stubs.as_deref());
                py_type_check(py, &code_str, script_name, combined_stubs.as_deref(), "type_stubs.pyi")?;
            }

            let (registry, callables) = build_extension_registry(py, ext_list)?;
            let runner = MontyRun::new_with_extensions(code_str, script_name, input_names.clone(), registry)
            ...
            return Ok(Self::from_parts_with_extensions(
                runner,
...
impl PyMonty {
    ...
    ///
    /// Extensions are not supported on this path — the off-GIL build can't
    /// touch Python extension callables. Use [`Self::from_parts_with_extensions`]
    /// from the sync `__new__` when extensions are provided.
    ...
    /// Assembles a `PyMonty` from its parts including host extension callables.
    ///
    /// Only used by the synchronous extension-aware `__new__` path, which
    /// stays on the GIL to read the Python-provided extension callables.
    pub(crate) fn from_parts_with_extensions(
        runner: MontyRun,
        script_name: String,
        input_names: Vec<String>,
        dc_registry: DcRegistry,
